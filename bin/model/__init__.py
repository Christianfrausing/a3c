...
class MountainCarModel(TemporalDifferenceAdvantageActorCritic):
    def __init__(self, observation_space, action_space, optimizer=torch.optim.Adam, optimizer_parameters={'lr' : 1e-3}, gpu=False):
    ...
    def rewards(self, states, next_states):
        position = states[:,0]
        next_position = next_states[:,0]
        velocity = states[:,1]
        next_velocity = next_states[:,1]
        return 100 * (
            (
                torch.sin(3 * next_position) * 0.0025 + 0.5 * next_velocity.pow(2)
            ) - (
                torch.sin(3 * position) * 0.0025 + 0.5 * velocity
            )
        )
    def loss(self, states, next_states, actions, rewards, t, entropy, discount_rate, temporal_difference_scale):
